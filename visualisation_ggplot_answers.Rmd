---
title: "Data visualisation with ggplot2 - Answers"
...
placing your cursor inside it and pressing <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>Enter</kbd> or
OS X: <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>Enter</kbd>. To execute a specific line within a code
chunk, placing your cursor on that line and pressing <kbd>Ctrl</kbd> + <kbd>Enter</kbd> or
OS X: <kbd>Cmd</kbd> + <kbd>Enter</kbd>.
...
Add a new chunk of code in your language of choice by clicking the
*Insert Chunk* button on the toolbar or by pressing <kbd>Ctrl</kbd> + <kbd>Alt</kbd> + <kbd>I</kbd>
or OS X: <kbd>Cmd</kbd> + <kbd>Option</kbd> + <kbd>I</kbd>.

When you save the notebook, a HTML file containing the code and output
will be saved alongside it (click the *Preview* button or press <kbd>Ctrl</kbd> +
<kbd>Shift</kbd> + <kbd>K</kbd> or OS X: <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>K</kbd> to preview the HTML file).
...
**TIP**: The R Markdown notebook (e.g.
*'visualisation_ggplot_answers.Rmd'*) can be viewed or edited in either
...
::: {style="background-color:#E4F8D6"}
**TIP**: When you save the notebook, a HTML file containing the code and
output will be saved alongside it (click the Preview button or press
<kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>K</kbd> or OS X: <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>K</kbd> to preview the HTML
file in the 'Viewer' tab). The preview shows you a rendered HTML copy of
the contents of the editor. Consequently, unlike Knit, Preview does not
...
like 'data-help'. The name is optional but if included, each code
...
kind of variable (int for integer, chr for character), and the first 10
entries in that variable. We will talk more about different data types
and structures later on.
...
plots from tabular data (data in a table format with rows and columns).
The **gg** in **`ggplot2`** stands for “grammar of graphics”, and the
package uses consistent vocabulary to create plots of widely varying
types. Therefore, we only need small changes to our code if the
...
tend to like data in the “long” or “tidy” format, which means each
...
Some languages, like Python, require certain spacing or indentation for
code to run properly. This isn’t the case in R, so if you see spaces or
indentation in the code from this lesson, it is to improve readability.
...
-   We use the `ggplot()` function to create a plot. In order to tell it
    what data to use, we need to specify the `data` argument. An
    argument is an input that a function takes, and you set arguments
    using the = sign.
...
we want to correspond to parts of the plot. We can specify the “mapping”
of variables to plot elements, such as x/y coordinates, size, or shape,
by using the `aes()` function. We’ll also add a comment, which is any
line starting with a `#`. It’s a good idea to use comments to organize
your code or clarify what you are doing.

::: {style="background-color:#E4F8D6"}
###TIP: comment or uncomment
RStudio makes it easy to comment or uncomment a paragraph. After selecting the lines you want to comment, press at the same time on
your keyboard <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd> or OSX:  <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd>. If you
...
press <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd> or OSX:  <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd>.
:::
...
closing bracket to end the geom_point function call. You can hit <kbd>Esc</kbd>
in the console to reset it.
...
Let’s try colouring our points according to the sampling plot type (plot
...
(`plot_type`) to a component of the ggplot2 plot (`colour`), we need to
...
Do you notice a difference in the colour scale compared to changing
colour by plot type? Why do you think this happened?
...
For Part 2, the colour scale is different compared to using colour =
plot_type because plot_type and year are different variable types.
plot_type is a categorical variable, so **`ggplot2`** defaults to use a
**discrete** colour scale, whereas year is a numeric variable, so
ggplot2 uses a **continuous** colour scale.
:::

### 2.7 Changing scales

The default discrete colour scale isn’t always ideal: it isn’t friendly
to viewers with colourblindness and it doesn’t translate well to
...
be colourblind and grayscale friendly. We can change scales by adding
`scale_` functions to our plots:
...
the way that `weight` is mapped to the x axis by using the
...
not the rectangular portions. This is because the `colour` only impacts
1-dimensional parts of a `ggplot`: points and lines. To change the
colour of 2-dimensional parts of a plot, we use `fill`:
...
them more legible is to wrap the text. We can do that by modifying the
`labels` for the x axis `scale`.
...
Let’s add `geom_point()` to visualise the raw data. We will modify the
...
which will add points with a bit of random noise added to the positions
...
we can assign it to a plot to a named `object`. We do this using the
...
In RStudio, typing <kbd>Alt</kbd> + <kbd>-</kbd> will write
`<-` in a single keystroke or OS X: <kbd>Option</kbd> +
<kbd>-</kbd>
:::

We will create an object called `myplot`. If you run the name of the
**`ggplot2`** object, it will show the plot, just like if you ran the
code itself.
...
::: {style="background-color:#E4F8D6"}
**TIP**: In the *Environment* tab you can see the current values for
variables in memory.
:::
...
The plotly R package serializes ggplot2 figures into Plotly's universal
graph JSON. plotly::ggplotly will crawl the ggplot2 figure, extract and
translate all of the attributes of the ggplot2 figure into JSON (the
colours, the axes, the chart type, etc), and draw the graph with
plotly.js.
...
Plotly graphs are interactive! Click on legend entries to toggle traces,
click-and-drag on the chart to zoom, double-click to autoscale,
...
my_interactive_plot <- ggplot(data = complete_old, mapping = aes(x = weight, y = hindfoot_length, colour = plot_type)) +
  geom_point(alpha = 0.2) +
  theme_bw()
