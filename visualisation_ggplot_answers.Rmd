...
You can execute code chunks by clicking the Run button within the chunk or by placing your cursor inside it and pressing <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>Enter</kbd> or OS X: <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>Enter</kbd>. To execute a specific line within a code chunk, place your cursor on that line and press <kbd>Ctrl</kbd> + <kbd>Enter</kbd> or OS X: <kbd>Cmd</kbd> + <kbd>Enter</kbd>.
...
You can add a new chunk of code in your language of choice by clicking the Insert Chunk button on the toolbar or by pressing <kbd>Ctrl</kbd> + <kbd>Alt</kbd> + <kbd>I</kbd> or OS X: <kbd>Cmd</kbd> + <kbd>Option</kbd> + <kbd>I</kbd>.

TIP: The R Markdown notebook (e.g. 'visualisation_ggplot_answers.Rmd') can be viewed or edited in either in 'Source' mode which shows the raw Markdown commands, or in the 'Visual' mode which shows how the Markdown will look in its formatted form. You can switch between the 'Source' and 'Visual' modes at any time using the tabs at the top of the notebook.

TIP: When you save the notebook, an HTML file containing the code and output will be saved alongside it. To preview the HTML file in the 'Viewer' tab click the Preview button or press <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>K</kbd> or OS X: <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>K</kbd>. The preview shows you a rendered HTML copy of the contents of the editor. Consequently, Preview does not run any R code chunks. Instead, the output of the chunk from when it was last run in the editor is displayed. There are other more sophisticated ways to display your notebook, such as using the Knit command, which we do not go into in this course.
...
Note: You'll see we have started naming some of our our 'code chunks', like 'data-help'. The name is optional but if included, each code chunk needs a distinct name. The advantage of giving each chunk a name is that it will be easier to understand where to look for errors, should they occur. Also, any figures that are created will be given names based on the name of the code chunk that produced them.
...
###TIP: comment or uncomment RStudio makes it easy to comment or uncomment a paragraph. After selecting the lines you want to comment, press at the same time on your keyboard <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd> or OSX: <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd>. If you only want to comment out one line, you can put the cursor at any location of that line (i.e. no need to select the whole line), then press <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd> or OSX: <kbd>Cmd</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd>.
...
Now try changing the plot so that the colour of the points vary by year. Do you notice a difference in the colour scale compared to changing colour by plot type? Why do you think this happened?
...
For Part 2, the colour scale is different compared to using colour = plot_type because plot_type and year are different variable types. plot_type is a categorical variable, so ggplot2 defaults to use a discrete colour scale, whereas year is a numeric variable, so ggplot2 uses a continuous colour scale.

2.7 Changing scales

The default discrete colour scale isn’t always ideal: it isn’t friendly to viewers with colourblindness and it doesn’t translate well to grayscale. However, ggplot2 comes with quite a few other colour scales, including the fantastic viridis scales, which are designed to be colourblind and grayscale friendly. We can change scales by adding scale_ functions to our plots:
...
Scales don’t just apply to colours- any plot component that you put inside aes() can be modified with scale_ functions. Just as we modified the scale used to map plot_type to colour, we can modify the way that weight is mapped to the x axis by using the scale_x_log10() function:
...
It looks like colour has only affected the outlines of the boxplot, not the rectangular portions. This is because the colour only impacts 1-dimensional parts of a ggplot: points and lines. To change the colour of 2-dimensional parts of a plot, we use fill:
...
Let’s add geom_point() to visualise the raw data. We will modify the alpha argument to help with overplotting.
...
Just as before, we can mapplot_type to colour by putting it inside aes().
...
In RStudio, typing <kbd>Alt</kbd> + <kbd>-</kbd> will write <- in a single keystroke or OS X: <kbd>Option</kbd> + <kbd>-</kbd>
...
This process of assigning something to an object is not specific to ggplot2, but rather a general feature of R. We will be using it a lot in the rest of this lesson. We can now work with themyplot object as if it was a block of ggplot2 code, which means we can use + to add new components to it.
...
Code may appear to be doing the same thing, when the details are actually quite different. The inner workings of ggplot2 are actually quite complex, since it turns out making plots is a very complicated process! Because of this, things that seem the same (removing parts of a plot), may actually be operating on very different components or stages of the final plot.
