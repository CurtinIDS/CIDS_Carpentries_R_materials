...
editor_options:
  markdown:
    wrap: 72
# always_allow_html: true # use if knitting to pdf
---
...
The R Markdown notebook (e.g. *'visualisation_ggplot_answers.Rmd'*) can
...
Lines starting with `#` are used to indicate a comment that is not
interpreted when the code chunk is run. It’s a good idea to use comments
to organize your code or clarify what you are doing.
:::
...
You'll see we have started naming some of our our 'code chunks', like
'data-help'. The name is optional but if used, every code chunk needs a
distinct name. The advantage of giving each chunk a name is that it will
be easier to understand where to look for errors, should they occur.
Also, any figures that are created will be given names based on the name
of the code chunk that produced them.
:::
...
'Comment' or 'uncomment' in RStudio makes it easy to comment or
uncomment a paragraph of commands so that they are not run. After
...
In the *Environment* tab you can see the current values for variables in
memory.
:::
...
graph JSON. `plotly::ggplotly` will crawl the ggplot2 figure, extract
and translate all of the attributes of the ggplot2 figure into JSON (the
