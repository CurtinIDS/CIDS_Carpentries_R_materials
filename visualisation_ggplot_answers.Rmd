...
This lesson is adapted from the [Data Analysis and Visualization in R for Ecologists 2 lesson](https://datacarpentry.github.io/R-ecology-lesson/visualizing-ggplot.html)
...
We start by loading the required packages that you installed during the setup. **`ggplot2`** is included in
the **`tidyverse`** package, so we'll load the tidyverse.
...
It looks like `colour` has only affected the outlines of the boxplot, not the rectangular portions. This is because the `colour` only impacts 1-dimensional parts of a `ggplot`: points and lines. To change the color of 2-dimensional parts of a plot, we use `fill`:
...
Mke the color of the points and outlines of the violins vary by `plot_type`, and set the `fill` of the violins to white. Try playing with the order of the layers to see what looks best.
