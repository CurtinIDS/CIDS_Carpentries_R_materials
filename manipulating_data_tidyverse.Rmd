...
We can see the contents of the data frame by typing its name: surveys.
...
We can also check just the top (the first 6 lines) of this data frame using the function head():
...
Notice that the class of the data is a `tbl_df`. This is referred to as a "tibble", the data structure is very similar to a data frame. A data frame is the de facto data structure for most tabular data, and what we use for statistics and plotting.

A data frame can be created by hand, but most commonly they are generated by the a function like read_csv(); in other words, when importing spreadsheets from your hard drive (or the web).
...
Having created a new table, it is therefore straightforward to explore the relationship between the weight of different genera within, and between, the
plots. The key point here is that we are still following a tidy data structure, but we have **reshaped** the data according to the observations of interest: average genus weight per plot instead of recordings per date.
...
The opposing situation could occur if we had been provided with data in then form of `surveys_spread`, where the genus names are column names, but we 
wish to treat them as values of a genus variable instead.

In this situation we are gathering the column names and turning them into a pair of new variables. One variable represents the column names as values, and
...
Note that now the `NA` genera are included in the re-gathered format. Spreading and then gathering can be a useful way to balance out a dataset so every
...
## Challenges 
1. Spread the `surveys` data frame with `year` as columns, `plot_id` as rows, and the number of genera per plot as the values. You will need to summarise before reshaping, and use the function `n_distinct()` to get the number of unique genera within a particular chunk of data. It's a powerful function! See `?n_distinct` for more.
