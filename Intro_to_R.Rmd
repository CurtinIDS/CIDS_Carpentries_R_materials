...
> In RStudio, typing <kbd>Alt</kbd> + <kbd>-</kbd> will write ` <- ` in a single keystroke in a PC, 
> while typing <kbd>Option</kbd> + <kbd>-</kbd> does the same in a Mac.
...
Here, the value of `weight_kg`is given to the `sqrt()` function, the `sqrt()` function
calculates the square root, and returns the value which is then assigned to
the object `ten`. This function is very simple, because it takes just one argument.
...
What are the values after each statement in the following? Run individual lines by putting your cursor on that line and pressing **Ctrl + Enter** or OS X: **Cmd + Enter**, this will evaluate only that line in the Console. 
You can check the content of an object by typing its name in the Console and pressing enter, or checking the Environment tab.
...
5. In R, we call converting objects from one class into another class
